...
Phase 1.1 的 m4_cpu 模型以 `a + b·kv` 擬合 softmax，其餘算子為查表常數[^cpu11]。門檻結果：中位 0.003（0.3%）、p95 0.018（1.8%）、max 0.034（3.4%），通過（softmax 3 點擬一線幾乎完美；其他算子為「預測 = 量測值」無殘差）。Phase 1.2 roofline 為 Phase 1.1 查表的超集（含尺寸外推能力），舊報告 `validation/reports/phase1.1/m4_cpu.json` 保留作歷史記錄[^cpu13]。
...
| 項目 | 狀態 |
|---|---|
| fp32 decode roofline（單 A76 核）| **就緒**：校準完成，中位 1.18%，門檻通過 |
