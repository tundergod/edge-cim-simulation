...
| # | 決策 | 來源 |
|---|---|---|
| D1 | **L4 重框**：ctx-1024 重現 = by-construction smoke-test（標明）；真驗證 = 各單元獨立計價（移除 decode 路徑的 L4-tuned `BW_eff` glue，改 CIM GEMV[L1] + M2 `mem_lpddr4x` **24.2 GB/s、eff 0.71、measured anchor**（**非** lpddr5 的 0.65 sim 值）+ overlap）+ 預測長文本 decode 與 4c/1c 趨勢（1.130/1.096/1.081） | 使用者 |
...
M3 event engine (simulator/runtime/events.py + resources.py)  heapq 迴圈：單元並發 + 共享 BW 競爭(knee)
   │ 每 op compute 由 Platform.compute_us() 取得（包 M1/M2/M4，引擎絕不自算；2.1 回 bare float、per-op provenance/bound 順延 #55）
...
4. `simulator/runtime/events.py`：自寫 heapq 迴圈 `run_dag(dag, platform, bw) -> token_latency_us`；每 op 經 `Platform.compute_us(node)` 取 compute（包 Phase-1 模型、引擎絕不自算；2.1 回 bare float、per-op provenance/bound 順延 #55），memory-bound op 經 `SharedBandwidth` 拉伸 → verify: `tests/test_event_engine.py` 手算 toy DAG（2 單元 1 依賴）命中已知 finish time；單元內串列不變式；concurrency-off == 各 latency 相加。
...
## Wave 2.2 — M6 排程器 + 混合精度（CimHetero）

> **實際 decompose（2026-06-14）**：Wave 2.2 拆成 **2.2a**（value-flow 基礎：fixture/DAG/oracle/domains/provenance，已 merge PR #60）+ **2.2b**（`docs/plans/phase-2.2b.md`：Scheduler ABC + value-based conversion-op + group-aware GPU-attention(R2) + **專案自身 CimHetero**（CIM-INT8 matmul × GPU-FP16 attention）+ doc sync）。**HeteroInfer/HPIM SOTA 重現順延至後續 wave**（非取消，ADR-0003 validation-first 仍成立）。下方原始 step 表保留為歷史參考。
