...
舊模型對每個 multi-tile GEMM 用單-tile 吞吐 tile-sum：knee 以下系統性**高估**（編譯器 fuse tile，比 sum 快），knee 以上 **嚴重低估 ~−65%**（完全沒有 cliff），整體 abs median 誤差 **{{cim.multitile_old_median_pct}}%**（max {{cim.multitile_old_max_pct}}%）。兩-regime cliff 模型（resident `lat=a+b·K·N`、spill `lat=2·K·N/floor`）對所有 Card-native 點：**舊 tile-sum median {{cim.multitile_old_median_pct}}% / max {{cim.multitile_old_max_pct}}% → 新 cliff median {{cim.multitile_new_median_pct}}% / max {{cim.multitile_new_max_pct}}%**，留出驗證 median **{{cim.multitile_holdout_pct}}%**（含 K-tiling 與 N-tiling 兩種；max {{cim.multitile_new_max_pct}}% 來自 pure-K-tiling 點，比 f(K·N) 略慢）。[^cim16] K·N > {{cim.native_envelope_m}}M（native envelope 之外）由 spill floor 外推（memory-bound 線性，低風險，`is_extrapolated=True` 標記）。**Phase 2 影響**：真實 decode FFN/lm_head GEMV（K·N≥16M）正落在 spill regime——舊 tile-sum 低估約 3×，新模型修正後 per-op M1 延遲對 Phase 2 事件引擎才正確。[^cim17]

> **K=2048 不是 contraction 維 K 的極限。** 圖 P1 的「2048」是**輸出寬度 W**（4 cores × 512 = 2048，N 方向的 native combined tile），不是 K（輸入/收縮維）的上限。K-staircase（固定 N=512、掃 K，M=1）顯示 **K 一路 native 編譯到至少 16384**（無 tiling，crossbar 以 16 banks × 512-in 在內部沿 K 累加，由 axcompile 處理），吞吐隨 K 上升（K=2048→106、K=12288→255 GOP/s）。所以 **K>2048 不需要我們做 tiling**；唯一的天花板還是 **K·N residency cliff**——K=16384/N=512（K·N=8.4M）一樣崩到 ~72 GOP/s，與 multi-tile 同一個 ~{{cim.cliff_knee_m}}M knee（證實 cliff 是 K·N 總量效應，與是 K-tiled 還是 N-tiled 無關）。**需要 tiling 的是輸出 N>2048**（圖 P1 的 N-staircase），不是 K。
...
[^cim16]: 來源 `validation/reports/phase1.5/cim_multitile.json` › `old_vs_new` = {old_tilesum_median:0.313,old_tilesum_max:0.724,new_cliff_median:0.033,new_cliff_max:0.119}，`resident_holdout.median_relerr` = 0.038（unique-K*N split，含 k_staircase；gate ≤0.10）
