...
<!-- gen:gate_summary (generated by tools/report/build_findings.py) -->
...
| **conversion (quant/dequant)** | scheduler-inserted (M6) | Phase-2 analytic modeling (no measurement): memory-bound cast costed by existing M2/M4 × M6 boundary crossings; precisions fixed per unit (ADR-0004 rev 2026-06-11) |
...
- **M6:** tunables include `precision_boundary_conversion_op_cost`, modeled analytically in Phase 2
  (ADR-0004 revised 2026-06-11): a dequant/requant is a memory-bound cast costed by the existing
  M2/M4 per-op models × the boundary crossings the scheduler inserts. Precisions are fixed per unit
  (CIM INT8, GPU FP16), so this is a Phase-2 modeling item, **not** a measurement gap.
...
- **conversion-op cost (M6):** Phase-2 analytic modeling item, not a measurement gap (ADR-0004 revised 2026-06-11) — memory-bound cast via existing M2/M4 models × M6 boundary crossings.
