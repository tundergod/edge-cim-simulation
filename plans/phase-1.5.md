...
**Axis A — prefill M-amortization:** dense sweep M∈{2..508} → affine `tile_lat = 40.84 + 0.0941·M` µs, fit max 3.1% / held-out 0.9%. Bridges to the M=1 decode anchor (41.83µs) within 3.5% — the old "no-bridge 2.5× gap" was a sparse-fit artifact; M-amortization is continuous M=1→508 (real compile wall ~M=510; M=511 fails).

**Axis D — native multi-tile RESIDENCY CLIFF (the headline).** M=1 native throughput rises smoothly to ~264 GOP/s up to a knee at **K·N ≈ 8.16M** (resident, weights in SRAM), then **collapses ~3.5× to a ~69.9 GOP/s floor** (DRAM spill). Knee sharply bracketed: resident max 7.93M ↔ spill min 8.39M. New 2-regime model — resident `lat=15.15+5.78·(K·N/1e6)` µs, spill `lat=2·K·N/(69.9·1e9)` — **held-out median 3.3% (unique-K·N split, incl. K/N staircases)**; vs all native pts it beats the old tile-sum **31%/100% → 2.8%/12.5%**. The old model was +31% over (sub-knee) and −65% under (supra-knee, the cliff it lacks). Matters for Phase 2: real decode FFN/lm_head GEMVs (K·N≥16M) live in the spill regime.
...
**Axis E — KV SPIKE = PROXY_INCONCLUSIVE (honest negative).** The K=1 memory-bound proxy eff_BW RISES with transfer size and never converges (9.6/17.0/26.7/35.9/44.4 GB/s for M=64..1024; M>=2048 fail). Every compilable point's working set (output N·M ≤ 2.1M elems) is BELOW the ~8M SRAM knee → the proxy is SRAM-staging-bound, never reaches the DRAM regime, so it CANNOT isolate the kv_append DRAM BW. The earlier 'M=256 ≈ M2' was coincidental. kv_append stays analytic on M2's measured DRAM streaming BW (24.2); the converged DRAM-bound on-card datapoint is the cliff spill regime (~35.5 GB/s weight-read, flat across K·N 8–17M, same order). DRAM-BW independent validation → Phase 2.
