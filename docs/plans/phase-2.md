...
接近真實的設計取自頂會模擬器先例：**ONNXim**（event-driven compute + cycle-level memory 分層，0.23% MAE vs RTL）、**LLMCompass**（ISCA'24，解析 per-op、4.1% e2e）、**GenZ**（解析 + 校準效率因子、5.82%）、**ASTRA-sim**（可換 analytical/cycle backend）、**Sniper**（interval、非 cycle-accurate、對真機 25% 仍可信）、**LLMServingSim**（per-iteration 結果重用，不展開整段生成）。我們的差異化 = silicon 校準的 per-unit + 單串流 + L4，這在這個子領域（HPIM/PAPI/CENT 皆 cycle-accurate 但未校準）是真正的優勢。
...
6. **`AllCimScheduler`**（2.2 移入 scheduler.py 前的最小映射；亦即 L4 baseline 與 D1 的獨立計價路徑）：decode 路徑**不**用 `BW_eff=18.33`，改 CIM GEMV(L1) + M2 `mem_lpddr4x` 0.65 效率 + overlap；CPU/GPU/kv 顯式並發 → verify: 各項 provenance 字串標明 silicon-fit 來源、無 L4-tuned 常數。
...
8. `validation/validate_e2e_l4.py`（**D1 重框**）：(a) **smoke-test** = AllCim ctx-1024 算術重現 closed-form，明標 by-construction、非證據；(b) **mechanism test** = 各單元獨立計價之和落在 L4 ≤15%（無 refit）+ 長文本 decode 點（LongBench，KV 流量增長）+ 4c/1c spread；6 vendor config + 長文本點 → verify: `reports/phase2/e2e_l4.json` 區分 smoke vs mechanism；mechanism ≤15% 且未動 integration 調參（Gate-6c）。
...
**Gate 2.1**: M5 過 oracle；M3 toy + ablation 方向正確；contention knee 趨勢對 4c/1c 一致（標 simulated）；L4 mechanism ≤15%；smoke/mechanism 明確分標。
...
4. **誠實修正**（plan-review 驗證的 committed 缺陷）：(a) [ADR-0006](../adr/0006-validation-bridging-extrapolation.md) 的「≈24 GB/s」與 committed `fit_BW_GBs:18.33` 是不同量、需釐清；(b) ADR-0006 的「4c/1c 1.31×→1.12×」與實測 `1.130/1.096/1.081` 不符——改成實測值或標來源；(c) m3.yaml `contention_knee` 移除「reproduce」字眼（無 on-platform 真值）→ verify: 每個改動數字 trace 到 committed JSON；`test_report_metrics.py` 同步。
