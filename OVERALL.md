...
| &nbsp;&nbsp;**Phase 1.2**（Wave 1，進行中） | **模組化「engine + 可換 spec」校準-analytic 元件層**（每個非-micro-benchmark 單元 = 模型引擎 + 可換 spec 模組，換型號=換 spec 檔）：**CPU**（instruction-count roofline + 多核/big.LITTLE）、**NPU**（解析 systolic-roofline，datasheet+HeteroInfer trend）、**記憶體**（analytic 全-spec LPDDR4/4X/5 + SRAM CACTI tier）、**GPU**（micro-benchmark + 可換 roofline 槽）、**CIM 雙拓樸**（Alpha 無 on-card DRAM / Metis Card 有）。**CIM-Card 重驗**：同一顆 AIPU 在 Card 上活著、`axrunmodel` 可量（已 SSH 驗證可行）→ CIM kernel **非凍結、可重驗**。非-測得標 `simulated`。報告 `docs/report/phase1.2/`。 | 完整、可換型號的模擬器（all components 齊備，輕引擎） |
