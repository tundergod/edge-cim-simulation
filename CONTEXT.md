...
| `simulator/specs/*.json` | **Unit/memory/topology specs** loaded by `simulator.specs.loader.load_spec`. Memory: `mem_lpddr4x` (24.2 measured anchor) / `mem_lpddr5` (33.3 sim). Topology (A/B/C, the `cim_topo_*` swap): `cim_topo_card` (on-card LPDDR4x, L4 anchor) / `cim_topo_alpha` (PCIe, no on-card DRAM, 911 µs floor, LLM-incapable) / `cim_topo_edge` (integrated SoC, LPDDR5×NoC, forward-looking). Plus cpu/gpu/npu/sram specs. Each self-labels `honesty`/`provenance`. Each topology's `cim_compute_params` (a params-file path) is the **CIM compute-engine swap surface** — resolved by `platform.py` into the `CimTileModel` (parallels the memory/topology swap); all three wired topologies point at the Metis `m1_cim.json`, and a topology declaring a non-Metis params file is tagged `simulated` (no non-Metis silicon) by `config._flag_provenance`. |
