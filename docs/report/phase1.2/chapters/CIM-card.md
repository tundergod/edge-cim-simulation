...
- **decode 交叉驗證（13 點）**：median `|rel_diff|` **4.8%**、p95 **9.6%**（square M=1,K=2048,N=2048：Card 200.5 vs Alpha 203.7 = 1.6%）。Card **系統性略低於** Alpha（12/13 點），小-N 差較大（~10%）、大-N 收斂（~1.5%）——同一 kernel 跨 SDK（Alpha v1.3.1 → Card v1.6）的小偏移。**在 decode 擬合容差內（median≤10%）→ Alpha 的 `G_eff(N,K)` 擬合 Card-confirmed、解凍、不重擬**（`m1_cim.json:decode_card_revalidation`）。
- **prefill M-amortization（新，Alpha 量不到）**：canonical 2048×2048 tile 的 `tile_lat(M) = 40.56 + 0.0942·M` µs（weight-load floor 40.6µs + per-column 0.094µs，asymptote ~89 TOPS），擬合 max rel-err **2.8%**。full GEMM = `n_tiles × tile_lat(M)`。`m1_cim_tile.dev_lat_us` 加 M>1 分支（M=1 decode 路徑**逐字不變**）：`dev_lat_us(128,4096,14336)` = 737µs vs 量測 724µs，**消除原本線性-M 外推的 65843µs（~80× 偏離）**。
- **TTFT 交叉驗證（`recompose_e2e.py`，P14）**：8B prefill（M=1024）擬合 compute = 0.245s = vendor TTFT 3.79s 的 **6.5%**，滿足物理界 `compute ≤ TTFT`（PASS）；舊線性-M decode-GEMV 估計 75s **違界 ~20×（REFUTED）**。prefill GEMM compute 由 `UNGATED` 升為 **BOUNDED**（TTFT 餘量 = weight-load 記憶體 + prefill attention + host overhead，Phase-2）。
