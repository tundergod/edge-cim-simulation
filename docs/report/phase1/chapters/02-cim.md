...
[^cim14] 5 個 full-GEMM 交叉驗證（K=4096, N∈{4096,14336}，M∈{128,256,64}）max rel_err = **{{cim.prefill_fullgemm_max_pct}}%**。[^cim15]

---

## 4　缺口 / 外推區

### 4.1　multi-tile：UNVALIDATED，系統性偏高 +{{cim.multitile_overpred_pct}}%

K·N > 4.19M（多-tile）的區域**只有 1 個原生點**：N=4096, K=1024，量測 37.1 µs，模型預測 **50.3 µs（+{{cim.multitile_overpred_pct}}%）**。[^cim16] 連續上升的 tile-sum 在真實硬體有 pipeline 效應時偏悲觀；但因板子離線（issues #2/#11/#17），無法補量。所有 K·N > 4.19M 的形狀（包括 1B/3B/8B 的 gate_up/down/q_o 大形狀，以及 lm_head N ≈ 128k/152k）**均為外推，模型標記 `is_extrapolated=True`**。[^cim17]
...
| multi-tile（K·N > 4.19M） | ⚠ 外推，`is_extrapolated=True`，+{{cim.multitile_overpred_pct}}% 偏高 |
