...
### L4 去循環（D1，本 wave 核心）
10. `AllCimScheduler`（最小映射，2.2 收進 scheduler.py）：matmul→CIM、attention→GPU、norm/rope/swiglu/softmax/residual/sampling→CPU、kv→mem；**decode 路徑不使用 e2e 擬合的 `BW_eff=18.33`**——CIM GEMV 走 `m1_cim_tile`(L1 校準) + 記憶體串流走 `m2_memory` `mem_lpddr4x`（**24.2 GB/s、eff 0.71、measured anchor**；**非** lpddr5 的 0.65 sim 值）+ CPU/GPU/kv 顯式並發 overlap → verify: 每 op provenance 標 silicon-fit 來源；`grep` 確認無 `18.33`/L4-tuned 常數注入 decode 路徑。
