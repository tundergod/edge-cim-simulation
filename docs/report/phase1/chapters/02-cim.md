...
> **修正先前假設：M_MAX=256「SRAM tiling 牆」高估了約一半。** dense sweep 編到 **M={{cim.prefill_m_max}}** 全部成功、無 error，**M≥512 才失敗**（`no_model_json`，3072/4096 等全失敗）——真正的牆在 **M=512**，約為舊假設 256 的 2×。所以牆是真的（SRAM 相關），只是位置被低估了一倍。prefill 校準範圍延伸到 M≤{{cim.prefill_m_max}}（M≥512 外推）。[^cim14]
...
| prefill M > {{cim.prefill_m_max}} | ⚠ 外推，`prefill_extrapolated=True`（真牆 M=512；max compiled {{cim.prefill_m_max}}） |
...
**Phase 1.5 已補（Card-native）：** multi-tile residency cliff（解鎖 8B FFN/lm_head decode 的 spill-regime 校準）、prefill dense M∈{2..{{cim.prefill_m_max}}}（舊 M_MAX=256 高估約 2×，真牆 M=512）、decode↔prefill 橋接、KV-append BW 驗證（見 3.x / 4.x）。
...
(a) M=1 native multi-tile 的 residency cliff：吞吐隨 K·N 平滑上升到 knee≈8.2M（權重 resident 於 SRAM），越過後崩落 ~3.5× 到 ~70 GOP/s 的 DRAM-spill floor；兩-regime 模型線貼合量測。(b) 對 Card-native 點的模型誤差：舊 tile-sum median 31% → 新 cliff 模型 2.4%（留出 2.9%）。(c) dense prefill M-sweep：仿射擬合（留出 {{cim.prefill_holdout_pct}}%），舊 M_MAX=256 假設高估約 2×（真牆在 M=512，max compiled {{cim.prefill_m_max}}），並橋接 M=1 decode anchor。(d) KV-cache isolation SPIKE：**INCONCLUSIVE**——K=1 proxy 的 eff_BW 隨傳輸量單調上升（{{kv.spike_bw_lo}}→{{kv.spike_bw_hi}} GB/s）且不收斂；其 working set 在編譯牆前都 < SRAM knee，即一直 SRAM-resident、從未進入 DRAM regime，故無法量到 kv-append 的 DRAM BW。kv-append 維持 analytic（用 M2 量測的 {{kv.spike_m2_bw}} GB/s），SPIKE 未能獨立佐證。
...
[^cim10]: 來源 `validation/reports/phase1.2/cim_prefill_fit.json` › `honesty` = "…DENSE Phase-1.5 canonical-tile sweep (M=2..448 all compile; M>=512 fail with no_model_json). The old M_MAX=256 'SRAM wall' was ~2x too low: a real wall exists at M=512, not 256."
...
[^cim14]: 來源 `validation/reports/phase1.2/cim_prefill_fit.json` › `affine_fit_tile_lat_us` = {a:40.26,b:0.0981,fit_basis:"M in {2..448} (25 dense pts)"}，`fit_quality.max_rel_err` = 0.032，`holdout.median_rel_err` = 0.006；`simulator/models/params/m1_cim.json` › `prefill_M_max` = 448（M>=512 編譯失敗）
