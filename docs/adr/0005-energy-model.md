...
## Decision
- **Spec-based per-component estimation** is the main model: **CIM** = vendor 15 TOPS/W × utilization (the M7 code folds utilization=1, i.e. **peak**, into the coefficient — `cim_J` has no separate utilization term; this is the optimistic corner for CIM and makes "memory dominates / CIM compute is cheap" *easier*, retained by-design only because the conclusion holds with ~240× headroom so realistic 1–50% utilization does not flip it — see `validate_m7_energy.py`), **cross-checked against the *form* of NeuroSim's CIM energy model** (NeuroSim validates to <1% chip-level error *after calibration* on a 40 nm **analog RRAM** macro; Metis is **digital SRAM-CIM**, so this is a model-form cross-check, *not* a silicon-grade source for our coefficient); **DRAM** = JEDEC pJ/bit × Ramulator-reported bytes; **PCIe** = spec pJ/bit × bytes; **NPU/GPU** = tech-node/datasheet; **CPU** = ARM datasheet × activity factor.
