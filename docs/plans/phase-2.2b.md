...
- **conversion 成本 = memory-bound cast,只算一次（ADR-0004,無新量測/新 param）**：`Platform.price(convert)` 回 **`latency_us=0`**(無 compute)、`source_model="convert"`;**真實成本只由 `bytes_streamed`(+`mem_domain`)經 M3 memory pool 收**——compute 與 memory **不雙收**(red test 斷言 convert delta == bytes/BW,且 price 的 compute=0)。`nbytes = n_elem × (read int8 1 + write fp16 2)`,其中 **`n_elem` = 被轉換 value(producer 輸出)的元素數**,來自 **OpNode 新欄位 `out_elems`**(build_token_dag 由 instantiated out_shape 設);**不可用 producer/consumer 的 `bytes_streamed` 代替**(那是 op 自身流量,非 value 大小)。**convert node 必有 `unit`**(scheduler 設,`_residency` 才能給 mem_domain)。`precision_boundary_placement` = tunable(哪個單元付)。
- **insert_conversions 回傳 rebuilt `Dag`(非就地 mutate)**：`Dag._succ` 在 construction 固定([dag.py](../../simulator/runtime/dag.py) `__init__`),只改 nodes/deps 會留 stale successor index、引擎跑錯 → 必須**用新 node list 重建 `Dag`**;且 **idempotent**(重複 assign/insert 不重複插 convert)。
...
4. **test 紅**：`tests/test_precision.py` — `insert_conversions(dag)` 在「恰一端 GPU + int8↔fp16」的 edge 插 `convert` OpNode。oracle = **獨立 edge-scan**(test 內直接掃 dag edges 用 precision contract+unit 判定,非呼叫 production code——避免循環),斷言 production `insert_conversions` 的插入點集合 == 獨立掃描集合。**oracle 須逐 (model, phase) 跑**(decode = 3/層:K+V dequant + S·V→O-proj requant;prefill = 2/層:無 kv_cache node——count phase-dependent,勿從 decode 導一次套用 prefill)。關鍵 case:**(a) AllCim DAG 插 0 個**(無 GPU node);**(b) 明確斷言 AllCim 的 cim(int8 matmul)→cpu(fp16 support) edge——有 precision delta 但無 GPU 端——插 0**(防止實作者誤改成「對 int8≠fp16 觸發」而在每條 cim→cpu 插 convert、靜默破壞硬 gate);**(c) CimHetero 的 kv_cache(int8,mem)→bmm(gpu) 與 S·V(gpu)→O-proj(cim) 確有 convert**。convert node `category="convert"`、有 `unit`(→`mem_domain` 經 `_residency`)、`bytes_streamed = producer.out_elems×(1+2)`(**由 producer `out_elems` 算,非 bytes_streamed**)、**`price()` compute=0**(cost 只走 memory);`insert_conversions` **回傳 rebuilt `Dag`**——test 斷言 `successors()`/deps/in_values/out_value 全一致 + acyclic + value 無懸空(`out_value==id`/`in_values==deps` 維持),且**重複呼叫不重複插**(idempotent);`domain_byte_audit` 互斥含 conversion_bytes(無雙算)。
5. **實作 綠**：`simulator/runtime/precision.py` — `insert_conversions(dag, *, placement)`:走 value-flow edge,符合「恰一端 GPU + int8↔fp16」者插 convert node、**用新 node list 重建並回傳 fresh `Dag`**(fan-out 處理明寫);convert node 賦 `unit`(per placement)→`_residency` 給 mem_domain。**`dag.py` OpNode 加 `out_elems` 欄位;`workload.build_token_dag` 由 instantiated out_shape 設**(conversion bytes 之來源)。`precision_boundary_placement`(改 `config.py`:加 SimConfig dataclass 欄位 **且** 在 `from_dict` 讀進來——`_KNOWN_SCHED` 已保留此 key 但目前未讀;預設 consumer 單元付;**僅 default+override,不加 sweep/CLI surface**)。`platform.price` 認 `category="convert"`→**`latency_us=0`**、`source_model="convert"`(成本只走 `bytes_streamed`,無新 param);`dag.CATEGORIES` 已含 `convert`。→ verify: test 綠;插入點集合 == 獨立 edge-scan;**convert delta == bytes/BW(compute 不收)**;AllCim 仍 0 conversion → **L4 不變**。
...
### E. doc sync（含 authoritative scope-truth,避免文件衝突）
10. 更新 `docs/adr/0003`(Scheduler ABC + AllCim/CimHetero plugins 已實作;**HeteroInfer/HPIM SOTA validation-first 順延至後續 wave**,非取消)、`docs/adr/0004`(conversion-op 已 analytic 實作 = memory-bound cast、無量測;**mixed-precision 品質 = D3 limitation,本 wave 只證成本不證品質**)、**`docs/plans/phase-2.md`**(原 Wave 2.2「M6+HeteroInfer」decompose 成 2.2a/2.2b,HeteroInfer deferral 落到 umbrella,免 scope-truth 衝突)、`CONTEXT.md` repo index(precision.py、scheduler classes、mixed_precision report)。→ verify: `build.py --strict` 綠;三份 authoritative docs(phase-2.md/ADR-0003/ADR-0004)與本 plan 的 scope 一致;報告/文件無 "validated mixed-precision quality" 字眼。

### F. Gate
11. 全 gate:(i) Scheduler ABC 純且單元測試過,AllCimScheduler == 2.2a 行為;(ii) **AllCim L4 rel_error byte-identical**(硬回歸,`abs(err−committed)<1e-4` vs 0.1073/0.0649/0.0311——非僅 ≤15%);(iii) conversion crossing count 可見(== fixture edge-scan)+ 由既有 memory 模型計價(**無新 param;price compute=0,成本只走 bytes,不雙收**);`insert_conversions` 回 rebuilt `Dag`(successors/deps 一致、idempotent);conversion bytes 由 `out_elems` 算;(iv) R2 composite group 總和 == 一次 attn_bmm_us(無雙算);(v) CimHetero 跑、conversion 成本可見、`mixed_precision.json` top-level **標 simulated**;(vi) mixed-precision **品質未宣稱**(D3 limitation,報告無 validated-mixed-precision 字眼);(vii) `.venv/bin/pytest tests/` + `docs/report/phase1-site/build.py --strict` 綠。

## Outputs
`simulator/runtime/{scheduler,precision,workload,platform,runner,config}.py`(+ `dag.py` 加 `out_elems`);`tests/test_{scheduler,precision,pricing}.py` + 擴充既有 tests;`validation/report_mixed_precision.py` + `reports/phase2/mixed_precision.json`;更新 `docs/plans/phase-2.md`、`docs/adr/0003`、`docs/adr/0004`、`CONTEXT.md`;`docs/plans/phase-2.2b.md`。
