...
- **Prefill GEMMs hit a compiler SRAM-tiling wall, NOT a DRAM-envelope wall:** a single ~2048×2048 tile compiles only at **M≤256**; M=512 / large-N FFN fail with `Could not find a tiling factor that fits the memory constraints` (L1≈4 MB). The Card's 16 GiB does **not** lift this (it is L1/L2, not BAR/device-DRAM). Measure the canonical tile at M∈{1,64,128,256} and extrapolate `n_tiles × tile_lat(M)` (the Alpha `SAFE_KN` method); `tile_lat(M)=a+b·M` captures the prefill M-amortization the M=1 decode points cannot.
