...
7. `simulator/runtime/events.py`：自寫 `heapq` 迴圈 `run_dag(dag, platform, bw, *, concurrency=True, contention=True) -> token_latency_us`；`OP_READY`/`OP_DONE`；每 op compute latency = `Platform.compute_us(node)`（包 Phase-1 M1/M2/M4 模型，引擎**絕不自算**；2.1 回 bare float，per-op provenance/bound 順延 #55）；memory-bound op 經 `SharedBandwidth` 拉伸 → verify: `tests/test_event_engine.py` —— 手算 toy DAG（2 單元 1 依賴）命中已知 finish time；單元內串列不變式；`concurrency=False` == 各 latency 相加；`contention=False` 移除 knee。
...
- per-op provenance/bound 管線（Gate-6c 責任追蹤；現 `Platform.compute_us()` 只回 bare float，**2.1 無 per-op provenance/bound**）— **issue #55**。
- **memory-domain attribution**：現在所有 `bytes_streamed>0` 都進同一個 on-card-DRAM `SharedBandwidth` 池，但 CPU support op 實際走 L1/L2/L3 cache（`cpu_rk3588.json`），非 DRAM。**M3 的 shared-BW attribution 尚非物理正確**（對 2.1 serial 數值影響小）— **issue #57**。
- DRAM 容量相依行為（residency/spill）+ on-chip SRAM tier wiring — **issue #58**；多 CIM 架構的 topology-parameterized memory model（PCIe±on-chip-DRAM / SoC-integrated）— **issue #59**（介面 flexibility 已鎖：pools 由 topology spec 建、domains topology-defined）。
