...
(a) SMOKE  : the Phase-1 closed-form (effective decode BW fit_BW_GBs=18.33 solved
             ...
             evidence.
(b) MECHANISM (the real, non-circular test, gated <=15%): the event engine prices
             each op independently — CIM-GEMV (L1 micro-benchmark) + the mem_lpddr4x
             24.2 GB/s memory wall + explicit CPU-support / attention / kv-cache
             traffic, with NO e2e-fitted bandwidth — and its decode tok/s must land
             within 15% of the measured 3x 1c vendor configs (8B is the hold-out).
