...
## Note (Phase 2.3): 14B/32GB extrapolation policy + capacity-formula divergence
The "~13B/32GB" stretch point is realised as **Qwen2.5-14B** (≈14B, in-family GQA), produced as a **mechanistic-model extrapolation** (engine path): an ADR-0007 FakeTensor meta-trace (no weights/silicon) feeds the same M5/M3 engine, gated by `validate_extrapolation_13b.py` (op_profile self-validation + structural oracle + cross-artifact count/bytes `oracle_check` + engine sanity). It is **`extrapolated`, never `validated`** (no 14B silicon) and carries THREE extrapolation flags: model-size (>8B), capacity (>16 GB SKU), and **CIM-shape** (per-op shapes beyond the M1 native measured envelope — a distinct dimension from "bigger model"). Capacity is checked by a **pinned validator-side formula** (`resident_footprint = runner per-op matmul footprint + KV + prefill-peak-activation`, where the weight term is the runner's `sum(row.bytes * row.count)` over decode matmul rows — `count==n_layers` so no layer double-count, NOT a deduplicated unique-weight count, and it OMITS the untied input-embedding table ~0.78 GB so the footprint is optimistic by ~0.78 GB; see the validator's `weights_GB_note`) that deliberately **diverges from the runner's weights-only feasibility gate** (`runner.py`): the validator surfaces the 16/32 GiB margins + the overflow crossover context, while the runner gate stays weights-only (locked); tightening the runner gate to the full footprint + spill behaviour is #58.
