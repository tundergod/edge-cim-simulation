...
4. **More cores do not fix decode.** 4-core/1-core speedup *decreases* with model size: 1B **1.130×** → 3B **1.096×** → 8B **1.081×** [median, `vendor_llm_int8.json`] (compute-bound would be a constant 4×). Bigger model → more bandwidth-bound. No 2-core artifact exists (`network.py` asserts `batch==cores`; ships 1c & 4c only).
